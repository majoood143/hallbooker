...
const Button = React.forwardRef(({
    ...
    // RTL classes
    const rtlClasses = isRTL ? 'flex-row-reverse' : 'flex-row';
    ...
    const renderIcon = () => {
        if (iconName) {
            ...
            const iconDirectionClass = getIconDirection(iconName, isRTL);
            
            const marginClass = children ? 
                (iconPosition === 'left' ? (isRTL ?'ml-2' : 'mr-2') : 
                    (isRTL ? 'mr-2' : 'ml-2')
                ) : '';
            ...
            return (
                <span style={{ color: iconColor || 'currentColor' }}>
                    <Icon
                        name={iconName}
                        size={calculatedSize}
                        className={`${marginClass} ${iconDirectionClass}`}
                    />
                </span>
            );
        }
